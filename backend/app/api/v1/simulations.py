...
def start_simulation(req: StartSimulationRequest, current_user: dict = Depends(get_current_user)):
    return simulation_service.start_simulation(current_user, req.domain, req.difficulty)
...
def submit_response(
    ...
    return simulation_service.submit_response(
        session_id, current_user, scene_number, req.response
    )


@router.post("/{session_id}/scenes", response_model=SceneResponse)
def next_scene(session_id: str, current_user: dict = Depends(get_current_user)):
    return simulation_service.request_next_scene(session_id, current_user)
