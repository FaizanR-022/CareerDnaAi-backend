...
export default function FeHeader({ difficulty, setDifficulty, a11yScore = 98, responsiveScore = "3/3 Breakpoints" }) {
    ...
    switch (difficulty) {
      case "easy":
        return {
          ...
          timerHtml: null,
          a11yText: `${a11yScore}%`,
          a11yIconClass: "text-primary",
          responsiveText: responsiveScore,
          responsiveIconClass: "text-primary",
        };
      case "hard":
        return {
          ...
          a11yText: `${a11yScore}%`,
          a11yIconClass: "text-error",
          responsiveText: responsiveScore,
          responsiveIconClass: "text-error",
        };
      case "medium":
      default: {
    ...
    <header className="bg-surface border-b border-outline-variant shadow-sm px-gutter py-4 flex justify-between items-center fixed top-0 left-0 right-0 z-50 h-20">
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="flex flex-col gap-1 w-32 md:w-36 transition-all duration-300">
          <div className="flex justify-between items-end">
            <div className="flex items-center gap-1 text-on-surface-variant">
              <span className={`material-symbols-outlined text-[16px] ${metrics.a11yIconClass}`}>
                accessibility_new
              </span>
              <span className="font-label-sm text-label-sm">Accessibility</span>
            </div>
            <span className="font-label-md text-label-md">{metrics.a11yText}</span>
          </div>
        ...
        <div className="flex flex-col gap-1 w-36 md:w-40 transition-all duration-300">
          <div className="flex justify-between items-end">
            <div className="flex items-center gap-1 text-on-surface-variant">
              <span className={`material-symbols-outlined text-[16px] ${metrics.responsiveIconClass}`}>
                devices
              </span>
              <span className="font-label-sm text-label-sm">Responsive Score</span>
            </div>
            <span className="font-label-md text-label-md">{metrics.responsiveText}</span>
          </div>
          <div className="h-2 bg-surface-container-high rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
              ...
              style={{ width: responsiveScore.includes("3/3") ? "100%" : responsiveScore.includes("2/3") ? "66%" : "33%" }}
            ></div>
          </div>
        </div>
      </div>
