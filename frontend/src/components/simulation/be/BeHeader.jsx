...
export default function BeHeader({ difficulty, setDifficulty, systemUptime = "99.99%", apiLatency = "45ms" }) {
  // Timer state (seconds remaining)
  const [secondsLeft, setSecondsLeft] = useState(() => {
    if (difficulty === "hard") return 240; // 4 minutes
    return 600; // 10 minutes default for medium
  });

  // Calculate elapsed percentage
  const totalSeconds = difficulty === "hard" ? 240 : 600;
    ...
    switch (difficulty) {
      case "easy":
        return {
          ...
          timerHtml: null,
          uptimeText: systemUptime,
          uptimeIconClass: "text-primary",
          latencyText: apiLatency,
          latencyIconClass: "text-primary",
        };
      case "hard":
        return {
          ...
          uptimeText: systemUptime,
          uptimeIconClass: "text-error",
          latencyText: "850ms", // Forced hard mode latency metric
          latencyIconClass: "text-error",
        };
      case "medium":
      default: {
        ...
        return {
          ...
          timerTextClass: isAmber ? "text-tertiary-container font-bold" : "text-primary font-bold",
          uptimeText: systemUptime,
          uptimeIconClass: "text-primary",
          latencyText: apiLatency,
          latencyIconClass: "text-primary",
    ...
    <header className="bg-surface border-b border-outline-variant shadow-sm px-gutter py-4 flex justify-between items-center fixed top-0 left-0 right-0 z-50 h-20">
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="flex flex-col gap-1 w-32 md:w-36 transition-all duration-300">
          <div className="flex justify-between items-end">
            <div className="flex items-center gap-1 text-on-surface-variant">
              <span className={`material-symbols-outlined text-[16px] ${metrics.uptimeIconClass}`}>
                dns
              </span>
              <span className="font-label-sm text-label-sm">Uptime</span>
            </div>
            <span className="font-label-md text-label-md">{metrics.uptimeText}</span>
          </div>
          <div className="h-2 bg-surface-container-high rounded-full overflow-hidden">
            <div
              ...
              style={{ width: metrics.uptimeText.includes("99.99") ? "100%" : "85%" }}
            ></div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 w-36 md:w-40 transition-all duration-300">
          <div className="flex justify-between items-end">
            <div className="flex items-center gap-1 text-on-surface-variant">
              <span className={`material-symbols-outlined text-[16px] ${metrics.latencyIconClass}`}>
                speed
              </span>
              <span className="font-label-sm text-label-sm">API Latency</span>
            </div>
            <span className="font-label-md text-label-md">{metrics.latencyText}</span>
          </div>
          <div className="h-2 bg-surface-container-high rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
              ...
              style={{ width: metrics.latencyText === "45ms" ? "100%" : "30%" }}
            ></div>
          </div>
        </div>
      </div>
